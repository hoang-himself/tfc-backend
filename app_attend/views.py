...
def add_attend(request):
    ...
    """
    # Get sched
    sched = get_object_or_404(Schedule, 'Schedule', pk=request.POST.get('sched_id'))
    
    # Get student
    try:
        student = get_object_or_404(MyUser, 'Student', uuid=request.POST.get('student_uuid'))
    ...
    if not sched.classroom in student.student_classes.all():
        raise ParseError('Student does not belong to the class of this session')
...
def list_attend(request):
    ...
    if attend is not None:
        attend = get_list_or_404(Attendance, 'Class', session__classroom__name=attend)
    ...
    if attend is not None:
        try:
            attend = get_list_or_404(Attendance, 'Student', student__uuid=attend)
        except ValidationError as message:
            raise ParseError({'detail': list(message)})
